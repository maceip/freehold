...
mod tests {
    ...
    fn handle_neighbors_transitions_to_connected() {
        ...
        let actions =
            sm.handle_message(Message::Neighbors { addrs: vec![] }, relay_addr(9999), now);
    ...
    fn neighbors_bounded_prevents_memory_exhaustion() {
        ...
        for batch in 0..10 {
            ...
            let addrs: Vec<Ipv4Addr> = (0..50)
                ...
                .collect();
            sm.handle_message(Message::Neighbors { addrs }, relay_addr(9999), now);
    ...
    fn rate_limit_drops_messages_in_state_machine() {
        ...
        for i in 0..150 {
            ...
            let actions = sm.handle_message(
                Message::Neighbors { addrs: vec![ip] },
                relay_addr(9999),
                now,
            );
            if !actions.is_empty() {
    ...
    fn two_relays_same_port_different_ips() {
        ...
        let actions = sm.handle_message(Message::Neighbors { addrs: vec![] }, addr2, now);
    ...
    fn anycast_ip_change_updates_relay() {
        ...
        sm.handle_message(Message::Neighbors { addrs: vec![] }, new_addr, now);
    ...
    fn exact_match_preferred_over_port_match() {
        ...
        sm.handle_message(Message::Neighbors { addrs: vec![] }, addr1, now);
