...
fn build_ebpf(opts: BuildEbpfOptions) -> anyhow::Result<()> {
    ...
    let src_file = project_root.join("ianal-c-ebpf/src/main.bpf.c");
    ...
    let out_file = out_dir.join("ianal-c.bpf.o");

    // Find clang
    let clang = find_clang();

    // Include path for BPF headers
    let include_dir = project_root.join("ianal-c-ebpf/include");
