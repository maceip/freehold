Part 1: Code & Build System
1. Directory Structure
Create a directory named ianal_c (or your preferred project name) and set it up as follows:
ianal_c/
├── Cargo.toml          # Workspace configuration
├── xtask/              # Build automation (standard for Aya)
│   ├── Cargo.toml
│   └── src/main.rs
├── ianal-c-ebpf/       # C (eBPF) Code
│   ├── Cargo.toml
│   └── src/main.bpf.c
└── ianal-c/            # Rust Userspace Loader
    ├── Cargo.toml
    └── src/main.rs


2. The eBPF Component (ianal-c-ebpf/src/main.bpf.c)
This file now encapsulates the quota management, registration logic, and the burst-aware reflector logic.
#include <linux/bpf.h>
...
3. The Rust Loader (ianal-c/src/main.rs)
Updated to load the program and optionally inspect the new quota map.
use aya::{
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut bpf = Ebpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/debug/ianal-c"
    ))?;
    #[cfg(not(debug_assertions))]
    let mut bpf = Ebpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/release/ianal-c"
    ))?;
...
Rust nightly (for eBPF compilation): rustup toolchain install nightly
